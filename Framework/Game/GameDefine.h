#ifndef GAMEDEFINE_INCLUDE_H
#define GAMEDEFINE_INCLUDE_H
...
struct UserBaseInfo {
	...
	int64_t userScore;    //用户积分
	uint32_t agentid;     //代理ID
	std::string lineCode; //线路
	uint32_t status;      //账号状态
	uint32_t ip;          //网络IP
	std::string location; //位置
	int64_t takeMaxScore; //最大进入分
	int64_t takeMinScore; //最小进入分
};
...
struct RoomInfo
{
    ...
    uint16_t    deskCount;            // table count.
...
struct RobotStrategyParam
{
    ...
    int64_t minScore;
    int64_t maxScore;
    std::vector<RobotStrategyArea> areas;
};
